...
{
const filteredUsers = ((name !== "") || (email !== "")) ? users
.filter(user => 
  {
    const fullName = `${user["first_name"]} ${user["last_name"]}`;
    return user["email"].toLowerCase().startsWith(email.toLowerCase()) && fullName.toLowerCase().startsWith(name.toLowerCase())
    ...
    { 
      ...
      const newUsers = paginate(filteredUsers, currentPage, pageSize);
        ...
        return (<React.Fragment>
            <h3>Manage Users</h3>
            {!loading && (<React.Fragment><div className="text-right">
            <div className="input-group w-50 mb-2">
                <input onChange={(event)=>{this.handleFilterOnChange(event)}} name="name" value={name} type="text" className="form-control" placeholder="Filter Name" />
                <input onChange={(event)=>{this.handleFilterOnChange(event)}} name="email" value={email} type="text" className="form-control" placeholder="Filter Email" />
                <button onClick={this.handleRequestOnChange} className="btn btn-sm btn-primary ml-1">Requests</button>
            </div>
            </div>
